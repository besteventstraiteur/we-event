    ...
    <header className="border-b border-vip-gray-200 py-3 px-3 sm:px-6 bg-white sticky top-0 z-10 w-full overflow-hidden">
      <div className="flex items-center justify-between max-w-full">
        {showBackButton && isNativeLike ? (
          <Button variant="ghost" size="sm" className="h-9 w-9 p-0 flex-shrink-0" onClick={onBackClick}>
            <ChevronLeft size={22} />
          </Button>
        ) : isNativeLike ? (
                  ...
                  <div className="pt-6 pb-2">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-9 pr-4 py-2 border rounded-full focus:outline-none focus:ring-2 focus:ring-primary"
                        autoFocus
                      />
                    </div>
                  </div>
                  ...
                  <div className="pt-6">
                    ...
                    <div className="space-y-4">
                      <div className="p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="text-sm text-gray-500">Vous avez reçu une demande de prix.</div>
                        <div className="text-xs text-gray-400 mt-1">Il y a 2 heures</div>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-lg">
                        <div className="font-medium">Rappel: Rendez-vous</div>
                        <div className="text-sm text-gray-500">Visite du lieu de réception demain.</div>
      ...
      {isNativeLike && (
        <div className="bottom-tabs fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-2 px-1 z-10">
...
const TabButton = ({ icon, label, isActive, href }: { icon: React.ReactNode, label: string, isActive: boolean, href: string }) => {
  return (
    <a 
      href={href} 
      className={`flex flex-col items-center justify-center px-2 ${isActive ? 'text-primary' : 'text-gray-500'}`}
    >
      {icon}
